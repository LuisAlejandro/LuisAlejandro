...
I've been on the field for more than ${new Date().getFullYear() - config.STARTYEAR} years but I'm always fascinated by new technology, which I try to integrate into new projects as soon as possible.
...
${config.SKILLS.map(({ label, items }) => {
  const skillset = `#### ${label}`;
  const skill = items.map(({ logo, label, color, labelColor, logoColor }) => {
